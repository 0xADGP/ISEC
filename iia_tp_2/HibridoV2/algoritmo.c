// Standard library includes for input/output, memory management, and mathematical operations
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include "algoritmo.h"
...
// Simple tournament selection between two random individuals
void tournament(pchrom pop, struct info d, pchrom parents) {
    // Iterate through the population
    for (int i = 0; i < d.popsize; i++) {
        // Select first random contestant
        int x1 = random_l_h(0, d.popsize - 1);
        int x2;
        // Select second random contestant (different from first)
        do {
            x2 = random_l_h(0, d.popsize - 1);
        } while (x1 == x2);

        // Choose winner based on fitness (lower is better)
        struct individual *winner = (pop[x1].fitness < pop[x2].fitness) ? &pop[x1] : &pop[x2];
        // Copy winner to parents population
...
void tournament_geral(pchrom pop, struct info d, pchrom parents) {
    // Array to store randomly selected positions
    int i, j, k, sair, best, *pos;
    
    // Allocate memory for tournament positions
    ...
    for (i = 0; i < d.popsize; i++) {
        ...
        for (j = 0; j < d.tsize; j++) {
            ...
            // Track best individual in tournament
            if (j == 0 || pop[pos[j]].fitness < pop[pos[best]].fitness) {
                best = j;
            }
        }
        // Copy tournament winner to parents
...
// Repair solution to ensure it meets problem constraints
