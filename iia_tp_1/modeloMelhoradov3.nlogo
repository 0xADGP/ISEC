...
to configurarAgentes
  ifelse posicionarAspiradoresEspecificos [
    let num-linhas-poderosos ceiling sqrt numAspiradoresPoderosos
    let num-colunas-poderosos ceiling (numAspiradoresPoderosos / num-linhas-poderosos)
    let espacamento-x-poderosos (max-pxcor - min-pxcor) / (num-colunas-poderosos - 1)
    let espacamento-y-poderosos (max-pycor - min-pycor) / (num-linhas-poderosos - 1)

    let posicoesPoderosos []

    foreach range num-linhas-poderosos [linha ->
      foreach range num-colunas-poderosos [coluna ->
        let x min-pxcor + (coluna * espacamento-x-poderosos)
        let y min-pycor + (linha * espacamento-y-poderosos)
        set posicoesPoderosos lput (list x y) posicoesPoderosos
      ]
    ]

    create-aspiradoresPoderosos numAspiradoresPoderosos [
      let posicao one-of posicoesPoderosos
      setxy (first posicao) (last posicao)
      set size 1.2
      set color cyan
    ...
    let num-linhas ceiling sqrt numAspiradores
    let num-colunas ceiling (numAspiradores / num-linhas)
    let espacamento-x (max-pxcor - min-pxcor) / (num-colunas - 1)
    let espacamento-y (max-pycor - min-pycor) / (num-linhas - 1)

    let posicoes []

    foreach range num-linhas [linha ->
      foreach range num-colunas [coluna ->
        let x min-pxcor + (coluna * espacamento-x)
        let y min-pycor + (linha * espacamento-y)
        set posicoes lput (list x y) posicoes
      ]
    ]

    create-aspiradores numAspiradores [
      let posicao one-of posicoes
      setxy (first posicao) (last posicao)
      set size 0.8
      set ticksEspera 0
...
to moveAspiradores
  ask aspiradores [
    ...
    ; Verificar a energia necessária para chegar ao carregador mais próximo
    moverParaCarregador
...
to-report encontrarCarregadorMaisProximo
  ; Verificar se o agente já conhece carregadores
  ifelse not empty? coordsCarregador [
    ; Criar um agentset a partir das coordenadas dos carregadores
    let carregadores-lista (map [coords -> patch-at (first coords) (last coords)] coordsCarregador)

    ; Remover patches que não existem
    let carregadores-agentset patch-set filter [p -> p != nobody] carregadores-lista

    ; Verificar se o agentset não está vazio
    ifelse any? carregadores-agentset [
      ; Encontrar o carregador mais próximo baseado na distância
      let destino min-one-of carregadores-agentset [
        distance myself  ; Usar a função distance
      ]

      show (word "Carregador mais próximo encontrado em: " destino)
      report destino  ; Retorna as coordenadas do carregador mais próximo
    ] [
      report nobody  ; Retorna "nobody" se o agentset estiver vazio
    ]
  ] [
    report nobody  ; Retorna "nobody" se não houver carregadores conhecidos
  ]
end



to moverParaCarregador
  ; Encontre o carregador mais próximo
  let destino encontrarCarregadorMaisProximo

  if destino != nobody [
    ; Usar uma busca simples para calcular o caminho até o carregador
    let caminho busca-simples patch pxcor pycor destino

    ; Se houver um caminho, mover o aspirador passo a passo
    if not empty? caminho [
      ; Calcular a energia necessária para percorrer o caminho
      let energiaNecessaria length caminho
      show (word "Energia necessária para chegar ao carregador: " energiaNecessaria)

      ; Verificar se a energia do aspirador é menor que a energia necessária
      if energia < energiaNecessaria [
        show "Energia insuficiente para mover até o carregador."
        stop  ; Para se não houver energia suficiente
      ]
      if energia = energiaNecessaria [
        ; Pega o próximo passo no caminho
        let proximo-passo first caminho

        ; Mover o aspirador para o próximo patch no caminho
        move-to patch (first proximo-passo) (last proximo-passo)

        ; Remover o primeiro passo do caminho (já foi percorrido)
        set caminho but-first caminho

        show (word "Aspirador moveu para: " (first proximo-passo) "," (last proximo-passo))
      ]
    ]
  ]
end
...
to-report busca-simples [origem destino]
  let fila (list (list [pxcor] of origem [pycor] of origem))  ; Fila de caminhos a explorar
  let visitados []                                            ; Lista de patches visitados

  while [not empty? fila] [
    ; Pegar o primeiro caminho da fila
    let caminho first fila
    set fila but-first fila

    let atual patch (first last caminho) (last last caminho)

    ; Se alcançou o destino, reporta o caminho
    if atual = destino [
      report caminho
    ]

    ; Marcar o patch atual como visitado
    set visitados lput atual visitados

    ; Definir os vizinhos manualmente
    let vizinhos (list
      patch-at 0 1  ; norte
      patch-at 0 -1 ; sul
      patch-at 1 0  ; leste
    ...
    foreach vizinhos [ vizinho ->  ; Aqui usamos "vizinho ->" para definir o item atual
      ; Verificar se o vizinho não é um obstáculo (não é branco) e não foi visitado
      if ([pcolor] of vizinho != white) and (not member? vizinho visitados) [
        ; Criar um novo caminho adicionando o vizinho ao caminho atual
        let novo-caminho (lput (list [pxcor] of vizinho [pycor] of vizinho) caminho)
        set fila lput novo-caminho fila
      ]
    ]
  ]
